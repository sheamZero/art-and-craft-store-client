...
const Categories = ({ items }) => {
    ...
    return (
        <div className="flex flex-col items-center gap-6 py-16">
            <h1 className="text-3xl text-paragraph font-bold">Explore Our Art & Craft Categories</h1>
            <p className="sm:w-1/2 text-center text-paragraph text-sm">
                Discover a diverse collection of handmade creations, carefully crafted across various styles and
                masterpieces—intricate sculptures, or unique decor, find inspiration and beauty in every category.
            </p>

            {/* category card */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 pt-5">
                    {items.map((item, idx) => (
                        <Link key={idx} className="flex flex-col items-center">
                            <div
